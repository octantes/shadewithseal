...
async function confirmarEdicion(obj, carpeta = null) {
  ...
  try {
    if (!carpeta) {
      ...
      // actualizar ids de archivos
      for (const archivo of carpetaUI.archivos) {
        archivo.id = `${limpio}/${archivo.nombre}`
      }
    } else {
...
export function useBiblioteca() {
  return {
    ...
    error,
    cargarBiblioteca,
    eliminarCarpeta: eliminarCarpetaDesdeUI,
    eliminarArchivo: eliminarArchivoDesdeUI,
    cancelarEdicion,
    confirmarEdicion,
    guardarNuevoArchivo,
    guardarNuevaCarpeta
